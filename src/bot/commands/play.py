...
from bot.bot import bot, queue_info, join, leave, ffmpeg_options, query_processor


@commands.hybrid_command(name="play", description="play some stuff, available music platforms: vk, youtube")
...
async def play(
    ...
    if voice_client:
        return await ctx.send(f"{len(queue['song_info']) - queue_size + imported_songs_amount} song(s) were added to the queue")
    else:
    ...
    while queue["song_info"]:
        ...
        response = f'{curr_song["parser"].name} Playing {curr_song["info"]["name"]}'
        response = response if not queue_imported else "Queue was succesfully imported\n" + response
        queue_imported = False
        await ctx.send(response)

        voice_client.play(discord.FFmpegPCMAudio(curr_song["info"]["params"]["link"], **ffmpeg_options))

        curr_song["info"]["start_time"] = datetime.now().replace(microsecond=0).timestamp()

        while voice_client.is_playing() or voice_client.is_paused():
            if not queue["repeat_current_song"]:
                if len(queue["song_info"]) > 1 and "link" not in queue["song_info"][1]["info"]["params"]:
...
async def prepare_new_song(queue:str, index:int)->None:
    while queue["song_info"] and "url" not in queue["song_info"][index]["info"]["params"]:
        song = queue["song_info"][index]
        info = await queue["song_info"][index]["parser"].search(song["info"]["name"], None)
        if not info:
            queue["song_info"].pop(index)
        else:
            queue["song_info"][index]["info"].update(info)
    
    while queue["song_info"] and "link" not in queue["song_info"][index]["info"]["params"]:
        info = await queue["song_info"][index]["parser"].get_song(queue["song_info"][index]["info"])
        if info:
