...
@commands.hybrid_command(name="repeat", description="Repeat current song\nuse the command again to stop the music from repeating")
async def command_repeat(ctx):
    ...
    if queue["repeat_current_song"]:
        ...
        await ctx.send(f"{queue['song_info'][0]['info']['name']} is no longer on repeat")
        return
    queue["repeat_current_song"] = True
    await ctx.send(f"{queue['song_info'][0]['info']['name']} is on repeat now")
