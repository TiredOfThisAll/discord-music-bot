    ...
    async def left_page_button_callback(self, interaction, button):
        ...
        for button in self.children:
            ...
            if button.custom_id == "right_page_button":
                button.disabled = self.page_number == ceil(len(self.queue["song_info"][1:]) / 10)
...
async def queue_embed(queue:dict, page_number:int=1)->discord.Embed:
    ...
    embed = discord.Embed(title=f"Queue page {page_number} of {max(1, page_count)}", colour=discord.Colour.blurple())

    song_cur_time = datetime.now().replace(microsecond=0) - datetime.fromtimestamp(queue["song_info"][0]["info"]["start_time"])

    song_duration = timedelta(seconds=queue["song_info"][0]["info"]["duration"])

    time_string = f"[{song_cur_time}/{song_duration}]"

    estimated_time = sum([song["info"]["duration"] for song in queue["song_info"]])
    ...
    embed.add_field(name="Current song", value=f"{queue['song_info'][0]['info']['name']}", inline=False)
    embed.add_field(name="Timebar", value=time_string)
    embed.add_field(name="Estimated time playing: ", value=estimated_time)

    None if len(queue["song_info"]) < 2 else embed.add_field(name="", value=f"```{get_queue_page(page_number, queue['song_info'][1:])}```", inline=False)

    return embed


def get_queue_page(page_number, urls):
    return "".join(list(map(lambda x: f"{(page_number - 1) * 10 + x[0] + 1}. {x[1]['info']['name']} \n", enumerate(urls[(page_number-1)*10:10*page_number]))))
