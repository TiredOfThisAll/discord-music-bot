...
class VK(BaseParser):
    ...
    searchFMT   = "https://i{server_id}.kissvk.com/api/song/search/do?origin=kissvk.com&query={query}"
    downloadFMT = "https://i{server_id}.kissvk.com/api/song/download/get/10/{artist}-{title}-kissvk.com.mp3?origin=kissvk.com&url={url}&artist={artist}&title={title}&index={song_id}"
    headers = {
    ...
    vk_song_xpath_duration = "../div[@class=\'audio_row__info _audio_row__info\']/div[@class=\'audio_row__duration audio_row__duration-s _audio_row__duration\']"
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=self.headers) as response:
                ...
                if data["songs"]:
                    ...
                    image = song["vkAlbumPictureUrl"] if "vkAlbumPictureUrl" in song else None
                    ...
                    # callback(SearchResult(song["artist"] + " - " + song["title"], int(song["duration"]), params), self)
                    if callback:
                        callback({
    ...
    async def process_url(self, url:str)->list:
        ...
        if "playlist" not in url:
            return
        check = re.search("audio_playlist(?P<playlist_id>\d+\D{1}\d+)", url)
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=headers) as response:
                ...
                for entry in tree.xpath(self.vk_song_xpath):
                    title = "".join(entry.xpath(self.vk_song_xpath_title + "/text()"))

                    artist = ", ".join(entry.xpath(self.vk_song_xpath_performer + "/text()"))
                
                    duration = datetime.strptime("".join(entry.xpath(self.vk_song_xpath_duration + "/text()")), "%M:%S")
    ...
    async def get_song(self, song_info:dict)->dict:
        ...
        download_url = self.downloadFMT.format(server_id=params["server"], artist=encartist, title=enctitle, url=encurl, song_id=params["index"])
